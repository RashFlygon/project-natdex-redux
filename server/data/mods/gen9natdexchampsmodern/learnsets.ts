...
function addMissingMoves(target: LearnsetTable, source: LearnsetTable) {
	...
	for (const id in sourceTable) {
		...
		if (!targetEntry.eventData && sourceEntry.eventData) {
			targetEntry.eventData = sourceEntry.eventData.map(event => ({
				...event,
				abilities: event.abilities ? [...event.abilities] : event.abilities,
				moves: event.moves ? [...event.moves] : event.moves,
			}));
		}
		if (sourceEntry.eventOnly !== undefined && targetEntry.eventOnly === undefined) {
			targetEntry.eventOnly = sourceEntry.eventOnly;
		}
